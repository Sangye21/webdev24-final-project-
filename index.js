...
const app = express();
const PORT = 3000;
const API_KEY = '9506b895a12d4004ad68580ccfdb70cf';
...
app.get('/competition/:id/scorers', async (req, res) => {
    try {
        ...
        const data = response.data.scorers; // Assuming the API returns an array of scorers
        ...
        res.render('top_scorers', { competitionName: competitionId, data: data });
    } catch (error) {
...
app.get('/competition/:id', async (req, res) => {
    try {
        ...
        // Render the league table page with the retrieved data
        res.render('league_table', { competitionName: response.data.competition.name, standings: standings });
    } catch (error) {
        console.error('Error fetching data from API:', error);
        res.status(500).send('Error fetching data from API');
    }
